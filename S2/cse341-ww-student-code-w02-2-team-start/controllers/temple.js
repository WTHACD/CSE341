...
exports.update = (req, res) => {
  ...
  Temple.findOneAndUpdate({ temple_id: temple_id }, req.body, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot update Temple with id=${temple_id}. Maybe Temple was not found!`,
        });
...
exports.delete = (req, res) => {
  ...
  Temple.findOneAndRemove({ temple_id: temple_id })
    .then((data) => {
      if (!data) {
        res.status(404).send({
